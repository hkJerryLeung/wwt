...
async function handleGoogleSignUp() {
  ...
  try {
    ...
    errorMsg.value = err instanceof Error ? err.message : 'Google sign up failed'
  }
}
</script>
  ...
  <div class="flex min-h-screen items-center justify-center bg-bp-primary bp-grid-bg px-4">
    <div class="w-full max-w-sm">
      ...
      <div class="mb-10 text-center">
        ...
        <p class="mt-2 text-sm text-bp-muted">{{ t('auth.register') }}</p>
      </div>

      <!-- Success message -->
      <div
        v-if="successMsg"
        class="mb-4 border border-bp-success bg-bp-success/10 p-6 text-center"
      >
        <p class="text-sm text-bp-success">{{ successMsg }}</p>
        <router-link
          to="/auth/login"
      ...
      <!-- Form -->
      <form v-else class="border border-bp-border bg-bp-surface p-8" @submit.prevent="handleRegister">
        <!-- Error -->
        <div
          v-if="errorMsg"
      ...
      <div class="mt-6 text-center">
        <span class="text-xs text-bp-muted">{{ t('auth.hasAccount') }} </span>
        <router-link to="/auth/login" class="text-xs text-bp-accent transition-colors hover:text-bp-accent-bright">
          {{ t('auth.login') }}
        </router-link>
      </div>
